...
data.1 <- data.raw %>%
  ...
  mutate(alpha = case_when(bet == "x" ~ 0.04,
                           TRUE ~ 0.025),
         slope = case_when(bet == "x" ~ 1.081,
                           bet == "a" ~ 1.012,
                           bet == "h" ~ 1.003)) %>%
  mutate(p = round(slope*(1/mean.odds) - alpha, 2),
         p.not = 1 - p) %>%
  select(-slope) %>%
  filter(p > 0)
...
data.2 <- data.1 %>%
  mutate(liability = stake,
         stake = liability/(max.odds - 1)) %>%
  mutate(xv = ((0.95 * stake * p.not) - (p * liability))/liability) %>%
  mutate(xv = round(xv, 4)) %>%
  mutate(date = as.Date(date, format = "%Y-%m-%d"))

# Probabilities - back bets
data.2 %>%
  mutate(laywin = ifelse(result != bet, 0, 1)) %>%
  mutate(p = round(p, 2)) %>%
  group_by(p) %>%
  summarise(pct = sum(laywin)/n()) %>%
...
# Check probabilities - lay bets
data.2 %>%
  mutate(laywin = ifelse(result != bet, 1, 0)) %>%
  mutate(p.not = round(p.not, 2)) %>%
  group_by(p.not) %>%
  summarise(pct = sum(laywin)/n()) %>%
  ggplot(aes(x=p.not, y=pct)) +
  geom_point(colour = "dodgerblue4") +
  theme_minimal() +
  geom_abline(intercept = 0, slope = 1) +
  coord_equal()
...
bets.data.2 <- bets.data %>%
  arrange(date) %>%
  mutate(profit = ifelse(result != bet, 0.95*stake, -1*liability)) %>%
...
# Select bets with any positive expected value - otherwise you are filtering to longer shots
# similarly, with higher stakes you do better, as you can afford the higher
# liability required for safer bets
monthly <- bets.data.2 %>%
...
# ---- Check back and lay combination ----

lay <- data.1 %>% mutate(back.lay = "lay")
back <- data.1 %>% mutate(back.lay = "back") %>%
  mutate(alpha = case_when(bet == "x" ~ 0.05,
                           TRUE ~ 0.05),
         slope = case_when(bet == "x" ~ 1.081,
                           bet == "a" ~ 1.012,
                           bet == "h" ~ 1.003)) %>%
...
bets.data.2 <- bets.data %>%
  ...
  mutate(liability = stake,
         stake = ifelse(back.lay == "back",
                        liability,
                        liability/(max.odds - 1)),
         xv = ifelse(back.lay == "back",
                     (0.95 * p * (max.odds - 1)) - (p.not),
                     ((0.95 * stake * p.not) - (p * liability))/liability)
         ) %>%
  group_by(id) %>%
  mutate(rank = row_number(desc(xv))) %>%
  ungroup %>%
...
bets.result <- bets.data.3 %>%
  ungroup %>%
  mutate(profit = case_when(back.lay == "back" & result == bet ~ 0.95 * stake * (max.odds - 1),
                            back.lay == "lay" & result != bet ~ 0.95 * stake,
                            TRUE ~ -1 * liability)) %>%
  group_by(date) %>%
...
result <- lapply(1:500, function(i) {
  sum(sample_frac(bets.result, matchrate)$profit)
})
hist(do.call(rbind, result))

# Does the amount of profit depend on which bets you randomly select?
...
simulation <- lapply(1:50, function(x) {
  ...
  bets.data.3 <- bets.data %>%
    arrange(date) %>%
    mutate(profit = case_when(back.lay == "lay" ~ ifelse(result != bet, 0.95*stake, -1*liability),
                              back.lay == "back" ~ ifelse(result == bet, 0.95*stake, -1*liability))) %>%
    select(-rank) %>%
    group_by(date) %>%
    summarise(profit = sum(profit),
              bets = n())
